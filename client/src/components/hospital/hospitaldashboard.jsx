...
const HospitalDashboard = () => {
    ...
    const addDepartment = async () => {
        if (!newDept.name.trim()) { showToast('error', 'Please enter a department name.'); return; }
        try {
            setDeptSaving(true);
    ...
    const saveBlood = async (id) => {
        const units = parseInt(bloodEdits[id]) || 0;
        try {
            setBloodSaving(id);
            const updated = await apiService.updateBloodInventory(id, {units_available: units});
            setBloodInventory(prev => prev.map(b => b.id === id ? updated : b));
            showToast('success', 'Blood inventory updated.');
        } catch (e) { showToast('error', 'Failed to update blood inventory.'); }
    ...
    const toggleEquip = async (id, current) => {
        try {
            ...
            showToast('success', `Marked as ${!current ? 'available' : 'unavailable'}.`);
        } catch (e) { showToast('error', 'Failed to toggle equipment.'); }
    };
    ...
    const saveEquipEdit = async () => {
        ...
        } catch (e) { showToast('error', 'Failed to update equipment.'); }
    ...
    const saveProfile = async () => {
        try {
            ...
            showToast('success', 'Hospital profile saved.');
    ...
    if (!hospitalId) {
        return (
            <div className="dashboard-container" style={{display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', flexDirection: 'column', gap: '8px'}}>
    ...
    return (
        <div className="dashboard-container">
            {toast && (
                <div className={`hd-toast hd-toast-${toast.type}`}>{toast.message}</div>
            )}

            {/* HEADER */}
            <div className="top-header hd-header-flex">
                ...
                {user && (
                    <div className="hd-user-chip">
                        <span className="hd-user-avatar">👩‍⚕️</span>
                        <div className="user-details">
                            <span className="user-name">{user.first_name || user.username}</span>
                            <span className="user-role">{user.role_display || user.role}</span>
                        </div>
                        <button className="hd-signout-btn" onClick={() => { logout(); navigate('/login'); }}>Sign Out</button>
                    </div>
                )}
            </div>
            ...
            <div className="hd-tab-content">
                                    ...
                                    {recentAlerts.length === 0 ? (
                                        ...
                                        <div key={alert.id} className={`alert ${alert.priority}`}>
                                            ...
                                            <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:'6px'}}>
                                                <span className={`hd-priority-badge hd-pr-${alert.priority}`}>{alert.priority?.toUpperCase()}</span>
                ...
                {activeTab === 'departments' && (
                    <>
                        <div className="hd-tab-header">
                            <div>
                                <h2 className="hd-tab-title">Department Management</h2>
                                <p className="hd-tab-sub">Manage bed availability and staffing by department</p>
                            </div>
                        ...
                        {showAddDept && (
                            <div className="hd-add-card card">
                                <h4 style={{marginBottom:'14px',color:'#111827'}}>New Department</h4>
                                <div className="hd-form-row">
                                    <div className="hd-field">
                                        <label>Name</label>
                                        <select value={newDept.name} onChange={e => setNewDept(p => ({...p, name: e.target.value}))}>
                                            <option value="">Select preset...</option>
                                            {DEPT_PRESETS.map(d => <option key={d} value={d}>{d}</option>)}
                                        </select>
                                        <input type="text" placeholder="Or type custom..." value={newDept.name}
                                               onChange={e => setNewDept(p => ({...p, name: e.target.value}))} style={{marginTop:'6px'}}/>
                                    </div>
                                    <div className="hd-field">
                                ...
                                <div className="hd-form-btns">
                                    <button className="hd-btn-primary" onClick={addDepartment} disabled={deptSaving}>
                        ...
                        {deptsLoading ? (
                            <div className="hd-loading"><div className="spinner"/>Loading departments...</div>
                        ) : departments.length === 0 ? (
                            <div className="hd-empty">
                                <div className="hd-empty-icon">🏥</div>
                                <p>No departments yet.</p>
                                <p style={{fontSize:'14px',color:'#9ca3af'}}>Add departments above or click <strong>⚡ Setup Defaults</strong> to initialize blood inventory and equipment.</p>
                            </div>
                        ) : (
                                ...
                                {departments.map(dept => (
                                    <div key={dept.id} className={`hd-table-row ${!dept.is_active ? 'hd-row-inactive' : ''}`}>
                                        {editingDept?.id === dept.id ? (
                                            <>
                                                <span>
                                                    <input className="hd-inline-input" value={editingDept.name}
                                                           onChange={e => setEditingDept(p => ({...p, name: e.target.value}))}/>
                                                </span>
                                                <span>
                                                    <input className="hd-inline-input hd-num" type="number" min="0" value={editingDept.total_beds}
                                                           onChange={e => setEditingDept(p => ({...p, total_beds: e.target.value}))}/>
                                                </span>
                                                <span>
                                                    <input className="hd-inline-input hd-num" type="number" min="0" value={editingDept.available_beds}
                                                           onChange={e => setEditingDept(p => ({...p, available_beds: e.target.value}))}/>
                                                </span>
                                                <span>
                                                    <input className="hd-inline-input hd-num" type="number" min="0" value={editingDept.doctors_on_duty}
                                                           onChange={e => setEditingDept(p => ({...p, doctors_on_duty: e.target.value}))}/>
                                                </span>
                                                <span>
                                                    <label className="hd-toggle-wrap">
                                                        <input type="checkbox" checked={editingDept.is_active}
                                                               onChange={e => setEditingDept(p => ({...p, is_active: e.target.checked}))}/>
                                                        <span className="hd-toggle-track"><span className="hd-toggle-thumb"/></span>
                                                        <span className="hd-toggle-label">{editingDept.is_active ? 'Active' : 'Inactive'}</span>
                                                    </label>
                                                </span>
                                                <span className="hd-row-btns">
                                                    <button className="hd-save-btn" onClick={saveDeptEdit} disabled={deptSaving}>
                                                        {deptSaving ? '...' : 'Save'}
                                                    </button>
                                            ...
                                            <>
                                                <span className="hd-dept-name-cell">{dept.name}</span>
                ...
                {activeTab === 'blood' && (
                    <>
                        <div className="hd-tab-header">
                            <div>
                                <h2 className="hd-tab-title">Blood Bank</h2>
                                <p className="hd-tab-sub">Track and update blood inventory for all blood types</p>
                            </div>
                            <button className="hd-btn-ghost" onClick={handleSetupDefaults} disabled={setupLoading}>
                        ...
                        {bloodLoading ? (
                            <div className="hd-loading"><div className="spinner"/>Loading blood inventory...</div>
                        ) : bloodInventory.length === 0 ? (
                            <div className="hd-empty">
                                <div className="hd-empty-icon">🩸</div>
                                <p>No blood inventory data found.</p>
                                <button className="hd-btn-primary" onClick={handleSetupDefaults} disabled={setupLoading} style={{marginTop:'12px'}}>
                                    ...
                                    {bloodInventory.map(b => {
                                        ...
                                        return (
                                            <div key={b.id} className="hd-blood-card" style={{background: s.bg, borderColor: s.border}}>
                                                <div className="hd-blood-type-label" style={{color: s.color}}>{b.blood_type}</div>
                                                ...
                                                <button
                                                    className="hd-blood-save-btn"
                                                    style={{background: s.color}}
                                                    onClick={() => saveBlood(b.id)}
                                                    disabled={bloodSaving === b.id}
                                                >{bloodSaving === b.id ? '...' : 'Save'}</button>
                                            </div>
                                        );
                                    })}
                                </div>
                                <div className="hd-blood-legend">
                ...
                {activeTab === 'equipment' && (
                    <>
                        <div className="hd-tab-header">
                            <div>
                                <h2 className="hd-tab-title">Equipment Management</h2>
                                <p className="hd-tab-sub">Track availability of critical medical equipment</p>
                            </div>
                            <button className="hd-btn-ghost" onClick={handleSetupDefaults} disabled={setupLoading}>
                        ...
                        {equipLoading ? (
                            <div className="hd-loading"><div className="spinner"/>Loading equipment...</div>
                        ) : equipment.length === 0 ? (
                            <div className="hd-empty">
                                <div className="hd-empty-icon">🔧</div>
                                    ...
                                    return (
                                        <div key={eq.id} className={`hd-equip-card ${eq.is_available ? 'hd-equip-on' : 'hd-equip-off'}`}>
                                            ...
                                            {isEdit ? (
                                                <div className="hd-equip-edit-area">
                                                    ...
                                                    <div className="hd-equip-edit-btns">
                                                        <button className="hd-save-btn" onClick={saveEquipEdit} disabled={equipSaving}>
                                                            {equipSaving ? '...' : 'Save'}
                                                        </button>
                                            ...
                                            <div className="hd-equip-footer">
                                                <button
                                                    className={`hd-avail-btn ${eq.is_available ? 'hd-avail-on' : 'hd-avail-off'}`}
                                                    onClick={() => toggleEquip(eq.id, eq.is_available)}
                                                >{eq.is_available ? '✓ Available' : '✗ Unavailable'}</button>
                                                {!isEdit && (
                ...
                {activeTab === 'profile' && (
                    <>
                        <div className="hd-tab-header">
                            <div>
                                <h2 className="hd-tab-title">Hospital Profile</h2>
                                <p className="hd-tab-sub">Update your hospital's contact information and capacity</p>
                            </div>
                        </div>

                        {profileLoading ? (
                            <div className="hd-loading"><div className="spinner"/>Loading profile...</div>
                        ) : (
                            <div className="hd-profile-card card">
                                <div className="hd-profile-grid">
                                    ...
                                    <div className="hd-field">
                                        ...
                                        <input type="number" min="0" value={profileForm.bed_capacity}
                                               onChange={e => setProfileForm(p => ({...p, bed_capacity: e.target.value}))}/>
                                    </div>
                                </div>
                                <div className="hd-profile-footer">
                                    <button className="hd-btn-primary hd-save-profile-btn" onClick={saveProfile} disabled={profileSaving}>
